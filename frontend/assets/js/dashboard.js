...
// ============================================
// CTIO NOTIFICATION SYSTEM
// ============================================

// Initialize CTIO Dashboard
function initDashboard() {
    console.log('🚀 CTIO Dashboard initializing...');
    ...
    checkForNewCTIONotifications();
    setInterval(checkForNewCTIONotifications, 10000); // Check every 10 seconds
    
    // Initialize other dashboard components
    initPerformanceChart();
    updateAccessoriesRevenue();
    
    // Update CTIO dashboard stats
    updateCTIODashboardStats();
    
    console.log('✅ CTIO Dashboard initialized with notification system');
}

// Update CTIO Dashboard Statistics
function updateCTIODashboardStats() {
    try {
        ...
        // Calculate stats
        const activeUsers = clients.filter(client => client.status === 'active').length;
        const expiredUsers = clients.filter(client => client.status === 'expired' || client.status === 'suspended').length;
        const totalBookings = bookings.length;
        const totalStaff = staff.length;
        
        // Calculate monthly income (example calculation)
        const currentMonth = new Date().getMonth();
        const currentYear = new Date().getFullYear();
        const monthlyPayments = clients.filter(client => {
            if (client.lastPayment) {
                const paymentDate = new Date(client.lastPayment);
        ...
        let monthlyIncome = 0;
        monthlyPayments.forEach(client => {
            if (client.packagePrice) {
                monthlyIncome += parseFloat(client.packagePrice) || 0;
            }
        });
        
        // Calculate accessory revenue (example calculation)
        let accessoryRevenue = 0;
        clients.forEach(client => {
            if (client.accessories && Array.isArray(client.accessories)) {
                client.accessories.forEach(accessory => {
                    accessoryRevenue += parseFloat(accessory.price) || 0;
                });
            }
        });
        ...
        const elements = {
            ...
            monthlyIncome: `Ksh ${monthlyIncome.toLocaleString()}`,
            accessoryRevenue: `Ksh ${accessoryRevenue.toLocaleString()}`,
        ...
        console.log('📊 CTIO Dashboard stats updated:', elements);
        
    } catch (error) {
        console.error('Error updating CTIO dashboard stats:', error);
    }
}

// Check for new notifications from supervisors
function checkForNewCTIONotifications() {
    try {
        const ctioNotifications = JSON.parse(localStorage.getItem('ctioNotifications') || '[]');
        const unreadNotifications = ctioNotifications.filter(n => !n.isRead);
        
        // Update notification badge
        updateCTIONotificationBadge(unreadNotifications.length);
        
        // Show recent unread notifications
        const recentNotifications = unreadNotifications.slice(0, 3); // Show latest 3
        displayCTIONotifications(recentNotifications);
        
    } catch (error) {
        console.error('Error checking CTIO notifications:', error);
    }
}

// Update notification badge
function updateCTIONotificationBadge(count) {
    const badge = document.getElementById('notificationBadge');
    if (badge) {
        if (count > 0) {
            badge.textContent = count > 99 ? '99+' : count;
            badge.style.display = 'flex';
            badge.className = 'absolute -top-2 -right-2 bg-red-500 text-white text-xs rounded-full h-6 w-6 flex items-center justify-center font-bold';
        } else {
...
// Display notifications in a dropdown or area
function displayCTIONotifications(notifications) {
    // This can be enhanced to show notifications in a dropdown
    // For now, we'll show recent notifications as toast messages
    notifications.forEach(notification => {
        if (shouldShowNotification(notification)) {
            showCTIONotificationToast(notification);
...
function shouldShowNotification(notification) {
    const shownNotifications = JSON.parse(localStorage.getItem('shownCTIONotifications') || '[]');
    return !shownNotifications.includes(notification.id);
}

// Mark notification as shown
function markNotificationAsShown(notificationId) {
    let shownNotifications = JSON.parse(localStorage.getItem('shownCTIONotifications') || '[]');
    shownNotifications.push(notificationId);
    // Keep only last 100 shown notifications
    if (shownNotifications.length > 100) {
        shownNotifications = shownNotifications.slice(-100);
    }
    localStorage.setItem('shownCTIONotifications', JSON.stringify(shownNotifications));
}

// Show notification toast
function showCTIONotificationToast(notification) {
    // Use SweetAlert for notification toast
...
// View all CTIO notifications
function viewAllCTIONotifications() {
    const ctioNotifications = JSON.parse(localStorage.getItem('ctioNotifications') || '[]');
    
    if (ctioNotifications.length === 0) {
        Swal.fire({
    ...
    const notificationsHtml = ctioNotifications.slice(0, 10).map(notification => {
        const timeAgo = dayjs(notification.createdAt).fromNow();
        const icon = notification.type === 'supervisor_ticket_created' ? '🎫' : '📬';
        const bgColor = notification.isRead ? 'bg-gray-50' : 'bg-blue-50';
        
        return `
            <div class="${bgColor} p-3 rounded-lg mb-3 border-l-4 ${notification.isRead ? 'border-gray-300' : 'border-blue-500'}">
                <div class="flex items-start justify-between">
                    ...
                    ${!notification.isRead ? '<div class="w-3 h-3 bg-blue-500 rounded-full ml-2 mt-1"></div>' : ''}
                </div>
            </div>
        `;
    }).join('');
    ...
    Swal.fire({
        title: '📬 CTIO Notifications',
        html: `
            <div class="text-left max-h-96 overflow-y-auto">
                ${notificationsHtml}
            </div>
            <div class="mt-4">
                <button onclick="markAllCTIONotificationsAsRead()" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm">
                    Mark All as Read
                </button>
            </div>
        `,
...
window.viewAllCTIONotifications = viewAllCTIONotifications;
window.markAllCTIONotificationsAsRead = markAllCTIONotificationsAsRead;
...
// ============================================
// ACCESSORIES SALES INTEGRATION
// ============================================

// Update accessories revenue from sales data
function updateAccessoriesRevenue() {
    try {
        const accessoriesData = JSON.parse(localStorage.getItem('monthlyAccessoriesRevenue'));
        const accessoryRevenueElement = document.getElementById('accessoryRevenue');
        
        if (accessoryRevenueElement && accessoriesData) {
            const formattedRevenue = `Ksh ${accessoriesData.total.toLocaleString()}`;
            accessoryRevenueElement.textContent = formattedRevenue;
            
            console.log(`Updated accessories revenue: ${formattedRevenue}`);
        } else {
            // Default value if no sales data
            if (accessoryRevenueElement) {
                accessoryRevenueElement.textContent = 'Ksh 0';
            }
        }
    } catch (error) {
        console.error('Error updating accessories revenue:', error);
        const accessoryRevenueElement = document.getElementById('accessoryRevenue');
        if (accessoryRevenueElement) {
            accessoryRevenueElement.textContent = 'Ksh 0';
        }
...
// Listen for accessories updates from sales system
window.addEventListener('accessoriesUpdated', (event) => {
    console.log('Accessories data updated:', event.detail);
    updateAccessoriesRevenue();
});

// Initialize accessories revenue on page load
document.addEventListener('DOMContentLoaded', () => {
    // Update accessories revenue when dashboard loads
    setTimeout(updateAccessoriesRevenue, 1000); // Delay to ensure elements are loaded
    
    // Set up periodic update every 30 seconds
    setInterval(updateAccessoriesRevenue, 30000);
});